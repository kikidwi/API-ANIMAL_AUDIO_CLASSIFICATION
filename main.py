...
def extract_manual_features(audio_array: np.ndarray, sr: int = 16000) -> np.ndarray:
    """
    Extract manual audio features as fallback if YAMNet is not available
    ...
    Returns:
        Feature vector (padded/truncated to 1024 dimensions)
    """
    try:
        features = []
...
def predict_animal_sound(audio_array: np.ndarray) -> Dict[str, Any]:
    """
    Predict animal sound from audio array
